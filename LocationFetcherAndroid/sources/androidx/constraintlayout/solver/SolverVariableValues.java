...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    public int indexOf(SolverVariable variable) {
        ...
        if (iArr22[key] != -1 && this.variables[iArr22[key]] == id) {
            return iArr22[key];
        }
        return -1;
    }
    ...
    private void addToHashMap(SolverVariable variable, int index) {
        int[] iArr;
        int hash = variable.f29id % this.HASH_SIZE;
        int[] iArr2 = this.keys;
        int key = iArr2[hash];
        if (key == -1) {
            iArr2[hash] = index;
    ...
    private void removeFromHashMap(SolverVariable variable) {
        ...
        if (key != -1) {
            ...
            while (true) {
                ...
                if (iArr3[key] == -1 || this.variables[iArr3[key]] == id) {
                    int[] iArr4 = this.nextKeys;
                    int currentKey = iArr4[key];
                } else {
                    key = iArr3[key];
                }
            }
            int[] iArr42 = this.nextKeys;
            int currentKey2 = iArr42[key];
            if (currentKey2 != -1 && this.variables[currentKey2] == id) {
                iArr42[key] = iArr42[currentKey2];
                iArr42[currentKey2] = -1;
            }
        }
    }

    private void addVariable(int index, SolverVariable variable, float value) {
        this.variables[index] = variable.f29id;
    ...
    public void put(SolverVariable variable, float value) {
        ...
        if (value > (-f) && value < f) {
            ...
            int j = this.head;
            for (int i = 0; i < count; i++) {
                if (this.variables[j] == variable.f29id) {
                    this.values[j] = value;
                    return;
                }
                if (this.variables[j] < variable.f29id) {
