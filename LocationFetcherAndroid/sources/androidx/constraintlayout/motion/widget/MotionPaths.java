...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f27x;
        float position3 = position2;
        float position4 = motionPaths.f28y;
        float endCenterX = motionPaths2.f27x + (f / 2.0f);
        float endCenterY = motionPaths2.f28y + (f3 / 2.0f);
        float pathVectorX = endCenterX - (f5 + (f2 / 2.0f));
        float pathVectorY = endCenterY - (position4 + (f4 / 2.0f));
        this.f27x = (float) ((int) ((f5 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f28y = (float) ((int) ((position4 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        ...
        this.mMode = 2;
        this.f27x = (float) ((int) (((motionPaths.f27x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f28y = (float) ((int) (((motionPaths.f28y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f)));
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f27x;
        float f6 = position2;
        float position3 = motionPaths.f28y;
        float endCenterX = motionPaths2.f27x + (f / 2.0f);
        float endCenterY = motionPaths2.f28y + (f3 / 2.0f);
        this.f27x = (float) ((int) ((f5 + ((endCenterX - (f5 + (f2 / 2.0f))) * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f28y = (float) ((int) ((position3 + ((endCenterY - (position3 + (f4 / 2.0f))) * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        this.height = (float) ((int) (f4 + (scaleY * scaleHeight)));
        this.mMode = 3;
        if (!Float.isNaN(keyPosition.mPercentX)) {
            this.f27x = (float) ((int) (keyPosition.mPercentX * ((float) ((int) (((float) parentWidth) - this.width)))));
        }
        if (!Float.isNaN(keyPosition.mPercentY)) {
            this.f28y = (float) ((int) (keyPosition.mPercentY * ((float) ((int) (((float) parentHeight) - this.height)))));
        }
        this.mKeyFrameEasing = Easing.getInterpolator(keyPosition.mTransitionEasing);
        this.mPathMotionArc = keyPosition.mPathMotionArc;
    }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f2 = motionPaths.width;
        float f3 = motionPaths.f28y;
        float f4 = position2;
        float position3 = motionPaths.height;
        float endCenterX = motionPaths2.f27x + (motionPaths2.width / 2.0f);
        float endCenterY = motionPaths2.f28y + (motionPaths2.height / 2.0f);
        float pathVectorX = endCenterX - ((f2 / 2.0f) + f);
        float pathVectorY = endCenterY - (f3 + (position3 / 2.0f));
        this.f27x = (float) ((int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f28y = (float) ((int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        ...
        float f6 = (float) ((int) ((motionPaths3.f27x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f27x = f6;
        float f7 = pathVectorX;
        float f8 = (float) ((int) ((motionPaths3.f28y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.f28y = f8;
        this.f27x = f6 + ((-pathVectorY) * perpendicular);
        this.f28y = f8 + (pathVectorX * perpendicular);
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f27x, points.f27x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f28y, points.f28y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f27x, this.f28y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
