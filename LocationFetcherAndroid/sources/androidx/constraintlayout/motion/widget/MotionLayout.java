...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setState(TransitionState newState) {
        if (newState != TransitionState.FINISHED || this.mCurrentState != -1) {
            ...
            int i = C00982.f25xabc7e4ac[oldState.ordinal()];
            if (i == 1 || i == 2) {
    ...
    private static class MyTracker implements MotionTracker {
        ...
        /* renamed from: me */
        private static MyTracker f26me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f26me.tracker = VelocityTracker.obtain();
            return f26me;
        }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0251R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0251R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, a.getResourceId(attr, -1));
                } else if (attr == C0251R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C0251R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C0251R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else {
                    int i2 = 0;
                    if (attr == C0251R.styleable.MotionLayout_showPaths) {
                    ...
                    } else if (attr == C0251R.styleable.MotionLayout_motionDebug) {
