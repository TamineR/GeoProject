...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f53mX = new float[numPoints];
            this.f54mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
                this.f53mX[i] = position[0];
                this.f54mY[i] = position[1];
            }
            if (((double) Math.abs(this.f53mX[0])) > 1.0E-5d || ((double) Math.abs(this.f54mY[0])) > 1.0E-5d || ((double) Math.abs(this.f53mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f54mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f53mX[0] + "," + this.f54mY[0] + " end:" + this.f53mX[numPoints - 1] + "," + this.f54mY[numPoints - 1]);
            }
            float prevX = 0.0f;
            int componentIndex = 0;
            int i2 = 0;
            while (i2 < numPoints) {
                float[] fArr = this.f53mX;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f53mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f53mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        if (xRange == 0.0f) {
            return this.f54mY[startIndex];
        }
        float[] fArr2 = this.f54mY;
